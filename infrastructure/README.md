...
4. Build `Dockerfile.web-lambda` and `Dockerfile.worker` for `linux/amd64` with provenance disabled, then push commit-tagged images to their ECR repositories. Lambda requires a single-architecture image manifest.
...
docker buildx build --platform linux/amd64 --provenance=false --file Dockerfile.worker --tag WORKER_REPOSITORY:COMMIT --push .
```
